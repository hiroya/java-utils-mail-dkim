...
class CanonicalizationParser {
    ...
    private State _accept(byte b) {
        switch (b) {
            case CR: return cr();
            case LF: return lf();
            case SP: case HT: return sp(b);
            default: return chars(b);
        }
    }
    ...
    private State cr() {
        switch (s) {
            case Default: break;
            case CR: writeLn(); break;
            case Space: if (!removeLineEndReducedSp) write(SP); break;
        }
    ...
    private State lf() {
        switch (s) {
            case Default: writeLn(); break;
            case CR: writeLn(); break; // \r\n
            case Space: if (!removeLineEndReducedSp) write(SP); writeLn(); break;
        }
    ...
    private State chars(byte b) {
        switch (s) {
            case Default: write(b); break;
            case CR: writeLn(); write(b); break;
            case Space: write(SP); write(b); break; // write single SP
        }
